...
> Scripts cover every user-facing flow on iOS. Steps marked **[SIM]** are simulator-specific notes.  
...
| UAT-19 | Settings — iCloud backup | `BackupSection` |
...
| UAT-28 | Accessibility — VoiceOver | All primary screens |
...
**Precondition:** On Screen 5. Face ID or Touch ID enrolled on simulator/device (or passcode as fallback).

| Step | Action | Expected | Result |
|------|--------|----------|--------|
| 4.1 | View Screen 5 | Explanation of biometric protection; Continue button | |
| 4.2 | Read body copy | Mentions Face ID / Touch ID | |
| 4.3 | Tap Continue | Face ID / Touch ID prompt appears (or passcode fallback) | |
| 4.4 | Authenticate successfully | Proceeds to Screen 6 | |
| 4.5 | Re-enter from reset; decline biometric | App still proceeds (graceful fallback to passcode) | |
...
| 9.7 | Tap "☁️ Cloud Backup" | [SIM] "Cloud Unavailable" alert with iCloud sign-in guidance | |
| 9.8 | [Device] Tap "☁️ Cloud Backup" | iCloud backup enables; success alert shown | |
| 9.9 | Check loading state during enable | Button shows "⏳ Enabling…" and is disabled while in progress | |
| 9.10 | Check success alert text | "Your vault will now back up automatically to iCloud." | |
...
| 11.6 | Long-press a document | iOS native action sheet appears (ActionSheetIOS) | |
...
| 14.10 | Tap Share | iOS share sheet appears with kit file | |
...
| 16.7 | Read cloud scenario card label | Shows "It's in iCloud Drive" | |
| 16.8 | Read cloud scenario card instructions | Mentions iOS Files app, Browse, iCloud Drive | |
| 16.9 | Read USB scenario card | Mentions "Files app" and "Lightning or USB-C adapter" | |
| 16.10 | View "Not sure?" amber panel | Contact vault creator / solicitor guidance visible | |
| 16.11 | Tap "Select .afterme File" | iOS Files app opens | |
| 16.12 | Navigate in Files app | Browse → On My iPhone or iCloud Drive; .afterme file selectable | |
| 16.13 | Tap help button ("Need help finding the file?") | Help modal slides up | |
| 16.14 | Read help modal intro | "the iOS Files app will open" | |
| 16.15 | Read help modal step 1 | "Tap Browse at the bottom right" | |
| 16.16 | Read help modal step 2 | "Check iCloud Drive or On My iPhone" | |
| 16.17 | Read help note | "Save to Files" tip for emailed files | |
...
| 16.20 | Read biometric line in vaultIntro | "Face ID or Touch ID protects access to the vault each time you open the app." | |
...
**Precondition:** iCloud signed in; vault backup exists in iCloud Documents (AfterMe folder).

| Step | Action | Expected | Result |
|------|--------|----------|--------|
| 17.1 | On WelcomeScreen tap "Restore My Vault" | SurvivorImportScreen opens (mode = restore) | |
| 17.2 | Follow restore prompts | iCloud backup located; decryption begins | |
...
| 18.2 | Read hint text (toggle ON) | "Face ID / Touch ID required to access your vault" | |
| 18.3 | Read hint text (toggle OFF) | "Biometric lock is disabled — vault access uses device passcode only" | |
| 18.4 | Toggle biometric off | Face ID / Touch ID prompt appears; toggle updates; hint changes | |
...
## UAT-19 · Settings — iCloud Backup

**Precondition:** In Settings. iCloud account signed in (or expect "not available" on simulator).

| Step | Action | Expected | Result |
|------|--------|----------|--------|
| 19.1 | View Backup section | Cloud Auto Backup toggle; provider label shows "iCloud" | |
| 19.2 | [SIM] Toggle Auto Backup on | "iCloud not available" alert with sign-in guidance | |
| 19.3 | [Device] Toggle on | iCloud backup enabled; toggle shows ON; "Last Backup" date updates | |
| 19.4 | [Device] Tap "Backup Now" | Progress indicator; success or error feedback | |
...
**Precondition:** In Settings. Sandbox Apple ID configured in Simulator → Settings → App Store.
...
| 21.4 | View legal footer (annual) | App Store cancel instructions shown | |
| 21.5 | Complete sandbox purchase | isPremium = true; PaywallScreen dismisses | |
| 21.6 | View Subscription section again | Shows premium status and product name | |
| 21.7 | Tap "Restore Purchases" | Restore prompt mentions Apple ID | |
...
| 22.2 | Read key storage label | Shows "Secure Enclave" | |
...
| 22.6 | View distribute step hint text | ".afterme file — save to Files, AirDrop, or USB drive" | |
...
| 23.2 | Read cloud backup FAQ answer | Mentions iCloud Documents and iCloud Keychain | |
| 23.3 | Read "lose phone" FAQ answer | Mentions iCloud Keychain for key recovery | |
...
| 28.10 | Biometric toggle in Security section | VoiceOver reads "Biometric Lock, switch, [on/off]" | |
...
| 30.4 | Launch app; test vault unlock | Face ID / Touch ID prompt appears; vault key loads; documents accessible | |
...
| Cloud Backup button shows loading state + iCloud success alert | VaultDashboardScreen | |
...
| vaultIntro shows "Face ID or Touch ID" | SurvivorImportScreen | |
| Help modal mentions "iOS Files app" and "iCloud Drive" | SurvivorImportScreen | |
| Security biometric hint shows "Face ID / Touch ID" | SecuritySection | |
| Recovery wizard distribute hint shows "AirDrop" | PersonalRecoveryWizard | |
...
*After Me UAT Test Scripts — iOS · v3.1 · March 2026*
